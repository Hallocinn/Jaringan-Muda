...
<style>
    ...
    * { font-family: 'Poppins', sans-serif; margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* TOMBOL EVALUASI AKHIR */
    .btn-evaluasi-akhir { background: linear-gradient(135deg, #2563eb 0%, #1e40af 100%); color: var(--white); box-shadow: 0 4px 15px rgba(37, 99, 235, 0.3); padding: 14px 28px; border-radius: 12px; font-weight: var(--fw-semibold); font-size: var(--fs-button); text-decoration: none; display: inline-flex; align-items: center; gap: 8px; transition: all 0.3s ease; border: none;}
    .btn-evaluasi-akhir:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(37, 99, 235, 0.4); }
...
{{-- ================= AREA PETUNJUK KUIS ================= --}}
    ...
    <h3 class="petunjuk-title">Perkembangan Jaringan Muda Tumbuhan</h3>
    ...
    <div class="hasil-box" id="hasilBox" style="display:none;">
        <div class="hasil-content">
            ...
            <div class="hasil-action" id="actionButtons">
                ...
                <a href="{{ route('evaluasi') }}" class="btn-evaluasi-akhir" id="btnMateriSelanjutnya">
                    Buka Evaluasi Akhir <i class="fas fa-trophy" style="margin-left: 8px;"></i>
                </a>
            </div>
...
<script>
    ...
    // MENDAPATKAN ID USER AGAR MEMORI SPESIFIK PER AKUN (KUIS 3)
    const userId = "{{ auth()->id() }}"; 
    const keyPercobaan = 'percobaanKuis3_user_' + userId;
    const keyJawaban = 'jawabanKuis3_Siswa_user_' + userId;
    ...
    async function mulaiKuis() {
        if(dataSoal.length === 0) {
            panggilAlertCustom({ judul: "Opps! Soal Kosong", pesan: "Guru belum menginput soal untuk Kuis 3 ini.", tipe: "danger" });
            return;
        }

        let percobaanKe = parseInt(localStorage.getItem(keyPercobaan));
        if (percobaanKe >= MAX_PERCOBAAN) {
            panggilAlertCustom({ judul: "Batas Percobaan Habis", pesan: "Kamu telah menggunakan semua kesempatan kuis Mu (3/3).", tipe: "danger" });
            return;
        }
    ...
    function cobaLagiKuis() {
        let percobaanKe = parseInt(localStorage.getItem(keyPercobaan));
        if (percobaanKe >= MAX_PERCOBAAN) {
            panggilAlertCustom({ judul: "Akses Ditolak", pesan: "Kesempatan Mu telah habis.", tipe: "danger" });
            return;
        }
    ...
    async function eksekusiSelesaiKuis() {
        ...
        try {
            await fetch("{{ route('kuis3.simpan') }}", {
                method: 'POST',
                headers: {
