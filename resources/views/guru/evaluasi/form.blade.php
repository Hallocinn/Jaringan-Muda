...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-2">
            <a href="{{ route('guru.evaluasi.index') }}" class="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 flex items-center justify-center transition-colors shadow-sm">
                <i class="fa-solid fa-arrow-left"></i>
            </a>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">
                {{ isset($soal) ? 'Edit' : 'Tambah' }} Soal Evaluasi
            </h2>
        </div>
        <p class="text-sm text-gray-500 ml-11">Manajemen butir soal evaluasi akhir dengan tampilan terstruktur.</p>
    </div>
    ...
    <form action="{{ isset($soal) ? route('guru.evaluasi.update', $soal->id) : route('guru.evaluasi.store') }}" method="POST">
        @csrf
        ...
        <div class="flex flex-col lg:flex-row gap-6 mb-6">
            ...
            <div class="w-full lg:w-5/12 bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <div class="mb-5">
                    ...
                    <textarea name="pertanyaan" rows="6" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 outline-none transition-all resize-y" placeholder="Tuliskan soal evaluasi di sini..." required>{{ $soal->pertanyaan ?? '' }}</textarea>
                </div>
...
<script>
    // --- SweetAlert2 Toast untuk Error Form ---
    @if($errors->any())
    ...
    // Eksekusi fungsi toggle saat halaman pertama kali dimuat (berguna saat mode Edit)
